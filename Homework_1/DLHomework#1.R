...
network <- keras_model_sequential() %>%
  ...
  layer_dense(units = 256, activation = "relu", initializer_he_normal()) %>%
  layer_dense(units = 256, activation = "relu", initializer_he_normal()) %>%
  layer_dense(units = 10, activation = "softmax")

network %>% compile(
  optimizer = "adam", # optimizer_adam(lr=.0001, decay = 1E-6),
...
# Train the model
network %>% fit(train_images, train_labels, epochs = 15, batch_size = 256, validation_split = 0.2)
